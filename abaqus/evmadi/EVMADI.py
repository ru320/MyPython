...
################### main Form
root = tk.Tk()
root.title('EVMADI ' + Verion + ' (Enhanced View Manager for Abaqus by DI)')
...
Results_Frame.grid(column=0,row=1)
for i, j in zip(range(5), range(10)):
    btn =  tk.Button(Results_Frame, text='Close', command=lambda:Close(), font='AvenirNextCondensed',bg='black',fg='white',image=pixelVirtual,compound="c",width = 115)
    btn.grid(column=i,row=j)

#- Buttons Grid
Buttons_Frame = tk.Frame(root, bg='red', width=450, height=50, pady=3)
Buttons_Frame.grid(column=0,row=2)
for i in range(5):
    for j in range(10):
        btn =  tk.Button(Buttons_Frame, text='Close', command=lambda:Close(), font='AvenirNextCondensed',bg='black',fg='white',image=pixelVirtual,compound="c",width = 115)
        btn.grid(column=i,row=j)
